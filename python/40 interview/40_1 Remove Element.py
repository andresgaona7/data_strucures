...
of val in the list in-place and returns the new length of the modified list.

The function should not allocate extra space for another list; instead, it should modify the input list in-place 
...
def remove_element(my_list, val):
    aux = 0
    for current in range(len(my_list)):
        if my_list[current] == val:
            aux += 1
        else:
            my_list[current - aux] = my_list[current]
            
    for i in range(len(my_list) - 1, len(my_list) - aux - 1, -1):
        del my_list[i]
          
    return len(my_list)

def remove_element_course(nums, val):
    i = 0
    while i < len(nums):
        if nums[i] == val:
            nums.pop(i)
