...
class BinarySearchTree:
    ...
    # The 'is_balanced' and 'inorder_traversal' methods will be used to test your code
    ...
    def __sorted_list_to_bst(self, nums, left, right):
        ...
        middle = (right - left) // 2
        subtree = Node(nums[middle])
        left = nums[left:middle]
        subtree.left = self.__sorted_list_to_bst(left, 0, len(left) - 1)
        right = nums[middle+1:right+1]
        subtree.right = self.__sorted_list_to_bst(right, 0, len(right) - 1)
        print(subtree.value, left, right)

        return subtree

    def __sorted_list_to_bst_course(self, nums, left, right):
        if left > right:
            return None
